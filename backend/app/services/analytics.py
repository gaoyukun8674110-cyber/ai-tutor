...
class AnalyticsService:
    ...
    def update_question_stat(self, question_id: int, is_correct: bool, time_spent: float, hint_count: int):
        """更新题目统计"""
        stat = self.db.query(QuestionStat).filter(
    ...
    def get_question_stats(self, question_id: int) -> Optional[Dict[str, Any]]:
        """获取题目统计"""
        stat = self.db.query(QuestionStat).filter(
            QuestionStat.question_id == question_id
        ).first()
    ...
    def get_problematic_questions(
        ...
        """找出有质量问题的题目"""
        problematic = self.db.query(QuestionStat).filter(
    ...
    def update_skill_stat(self, skill_id: str, skill_name: str, is_correct: bool, time_spent: float):
        """更新知识点统计"""
        stat = self.db.query(SkillStat).filter(
            SkillStat.skill_id == skill_id
        ).first()
    ...
    def get_system_stats(
        ...
        # 活跃用户数
        active_users = self.db.query(func.count(func.distinct(BehaviorLog.user_id))).filter(
        ...
        total_sessions = self.db.query(func.count(TrainingSession.id)).filter(
            TrainingSession.created_at >= since_str,
        ).scalar() or 0
        
        # 总答题数
        total_answers = self.db.query(func.count(StudentAnswer.id)).filter(
