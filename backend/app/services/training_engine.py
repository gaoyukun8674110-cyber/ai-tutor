...
from app.models.session import TrainingSession, SessionQuestion, SessionStatus, LearningGoal
...
class TrainingEngine:
    ...
    def get_next_question(
        ...
        # 获取已做过的题目
        answered_questions = self.db.query(SessionQuestion).filter(
    ...
    def submit_answer(
        ...
        # 更新 SessionQuestion
        session_question = self.db.query(SessionQuestion).filter(
            SessionQuestion.session_id == session_id,
        ...
        # 记录答题
        session_question_obj = self.db.query(SessionQuestion).filter(
            SessionQuestion.session_id == session_id,
            SessionQuestion.question_id == question_id,
        ).first()
    ...
    def _get_recent_answers(self, session_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        ).order_by(StudentAnswer.created_at.desc()).limit(limit).all()
