...
from app.models.question import (
    Question, QuestionTag, QuestionSkill, PrerequisiteSkill,
    QuestionType, QuestionStatus, DifficultyLevel
)
...
class QuestionBankService:
    ...
    def search_questions(
        ...
        # 按知识点过滤
        if skill_ids:
            query = query.join(QuestionSkill).filter(
    ...
    def get_standard_solution(self, question_id: int) -> Optional[Dict[str, Any]]:
        """获取标准解和分步解"""
        question = self.get_question(question_id)
        if not question:
            return None
